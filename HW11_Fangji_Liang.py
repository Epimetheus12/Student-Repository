...
class Repository:
    ...
    def __init__(self, dir_path, suffix='.txt', db="HW11_Fangji_Liang.db"):
        '''__init__ magic method'''
    ...
    def print_pre_inst(self):
        ...
        if len(self.students) > 0:
            ...
            for cwid, inst in sorted(self.instructors.items()):
                for cour, num in sorted(inst.teach.items()):
                    pt.add_row([cwid, inst.name, inst.dep, cour, num])
            return pt
        else:
            raise ValueError("No data in instructors flies.")

    def print_pre_inst_2(self):
        '''print pretty table of Instructors Summary by database'''
        print("Instructors Summary:(From Database)")
        pt = PrettyTable(
            field_names=['CWID', 'Name', 'Dept', 'Course', 'Students'])
        for cwid, name, dept, cour, num in self.instructor_table_db(os.path.join(self.dir_path, self.db)):
            pt.add_row([cwid, name, dept, cour, num])
        return pt

    def instructor_table_db(self, db_path):
        '''excute sql into database and retrive data when quering'''
        db = sqlite3.connect(db_path)
        query_1 = "DROP TABLE IF EXISTS instructor_summary;"
        db.execute(query_1)
        query_2 = "CREATE TABLE instructor_summary(\
                InstructorCWID TEXT,\
                Name TEXT,\
                Department TEXT,\
                Course_Taught TEXT,\
        ...
        db.execute(query_2)
        query_3 = "INSERT INTO instructor_summary\
        ...
        try:
            ...
            raise sqlite3.OperationalError(
                f"Check {db_path}, table not found in this database")

        query_4 = "select * from instructor_summary"
        return db.execute(query_4)
...
def main():
    '''test file'''
    _ = Repository('C:/Users/70753/Desktop/MyPython/HW11-Test')
